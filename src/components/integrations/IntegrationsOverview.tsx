...
const integrations = [
  ...
  },
  {
    ...
    description:
      "Track traffic and article performance with privacy-friendly analytics.",
    icon: (
  ...
  },
  {
    ...
    description:
      "Sign in securely and connect your Google account for onboarding and data access.",
    icon: (
      <svg className="h-7 w-7" viewBox="0 0 24 24" fill="none">
        <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4" />
        <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
  ...
  },
  {
    name: "Stripe Billing",
    description:
      "Manage trial activation, subscription billing, and plan status securely.",
    icon: (
      <svg className="h-7 w-7" viewBox="0 0 24 24" fill="#635BFF">
        <path d="M13.976 9.15c-2.172-.806-3.356-1.426-3.356-2.409 0-.831.683-1.305 1.901-1.305 2.227 0 4.515.858 6.09 1.631l.89-5.494C18.252.975 15.697 0 12.165 0 9.667 0 7.589.654 6.104 1.872 4.56 3.147 3.757 4.918 3.757 7.076c0 4.72 2.891 6.395 6.572 7.912 2.342.967 3.058 1.631 3.058 2.564 0 .963-.831 1.545-2.281 1.545-1.925 0-4.838-1.032-6.687-2.227L3.5 22.406C5.266 23.452 8.18 24 10.818 24c2.619 0 4.777-.652 6.263-1.928 1.603-1.378 2.42-3.344 2.42-5.628 0-4.853-2.95-6.497-5.524-7.294z" />
      </svg>
    ),
  },
];
      ...
      <div className="mx-auto max-w-[1200px] px-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {integrations.map((item, i) => (
            <div
              key={item.name}
              className={`reveal reveal-delay-${(i % 4) + 1} flex items-start gap-4 rounded-xl border border-border-light bg-white p-5 transition-all duration-300 hover:border-border hover:shadow-sm`}
            >
